...
describe('Button', () => {
	it('renders button text correctly', () => {
		...
		const { getByText } = render(
			<Button umami='test Button'>{buttonText}</Button>,
		)
	...
	it('calls onClick handler when clicked', () => {
		...
		const { getByTestId } = render(
			<Button umami='test Button' onClick={onClickMock}>
				Click me
			</Button>,
		)
	...
	it('renders with disabled styles when disabled prop is true', () => {
		const { getByTestId } = render(
			<Button umami='test Button' disabled>
				Click me
			</Button>,
		)
		const buttonElement = getByTestId('submit-button-1')
	...
	it('renders with enabled styles when disabled prop is false', () => {
		const { getByTestId } = render(
			<Button umami='test Button' disabled={false}>
				Click me
			</Button>,
		)
		const buttonElement = getByTestId('submit-button-1')
